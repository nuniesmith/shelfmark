...
container while the API and worker services are being built.
...
The Discord bot lives in its own repository now,
[nuniesmith/discordarr](https://github.com/nuniesmith/discordarr), beside the
movie, TV and music commands, so one bot serves everything. Its book commands
(`/library`, `/request`, `/downloads`, `/job`, `/scan`, ...) are thin callers
of this API. What they do, how they are configured, and why they are built the
way they are (paging, confirming a large grab, the fail-closed role allow-list)
is documented there, under
[Book commands: design notes](https://github.com/nuniesmith/discordarr#book-commands-design-notes).

What stays here is the API's side of that conversation.
...
a dune buggy (5010) before it returns a single book. `/api/v1/releases/search`
takes a `media_type` (`audiobook` or `ebook`, which is what the bot's
`/request` sends) and turns it into an explicit category set for that type —
it never searches every category. Verified against the live indexer:
filtering to `book_only`'s 7000 alone returns zero audiobooks across a
103-result sample; audiobooks are under `3030` (Audio/Audiobook, standard
Newznab) and `100064` (this indexer's own AudioBook category).
...
The route still has its older `book_only` flag (default `true`, searches
`PROWLARR_BOOK_CATEGORIES`) for any caller that only wants "books,
generically" without picking a type. `media_type` sits beside it and takes
precedence when set — pass `categories` explicitly to bypass both.
...
`SHELFMARK_EBOOKS_ROOT` itself: `/api/v1/ebooks/search` walks the ebooks root,
matching on author, title, and filename, and `/api/v1/ebooks/{id}/download`
serves one file. When a book has more than one file (an epub next to a pdf,
...
- A search result's id is an opaque token, never a filesystem path. The
  ...
  nothing.
- Discord refuses attachments over 10 MB on an unboosted server, so for a
  bigger book the bot asks this API for a **download link** instead
  (`POST /api/v1/ebooks/{id}/link`). The link works for one book, expires (24
  hours by default), and is served at `/dl/<token>`, so it needs
  `SHELFMARK_PUBLIC_URL`. The token is signed with a key derived from
  `SHELFMARK_API_TOKEN`, so rotating that token revokes every outstanding
  link. Without a public URL no link is issued, and the bot says the book is
  too big, naming it and its size.

| Variable | What it does |
|---|---|
| `SHELFMARK_API_TOKEN` | The bearer token every caller, the bot included, presents. Download links are signed with a key derived from it. |
| `SHELFMARK_PUBLIC_URL` | Where people reach the API, e.g. `https://shelfmark.example.org`. The base of every download link. Unset: no links. |
...
| `PROWLARR_BOOK_CATEGORIES` | Categories `media_type=ebook` (and `book_only`) restricts to. Default `7000`, since a typical indexer advertises the general Books bucket rather than 7020 (EBook) specifically. |
| `PROWLARR_AUDIOBOOK_CATEGORIES` | Categories `media_type=audiobook` restricts to. Default `3030,100064` (standard Newznab Audio/Audiobook plus this indexer's own AudioBook category) — verified against the live indexer; `book_only`'s 7000 alone returns zero audiobooks. |
...
- The `shelfmark-worker` container has its own Docker `healthcheck` now
  ...
  `shelfmark-api`/`shelfmark-worker`) installed by
