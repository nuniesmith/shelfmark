...
10. The organizer generates a preview manifest. **[x] implemented** — `organize_apply`'s plan/manifest, now auto-enqueued on a verified transfer.
...
12. Files are atomically moved into the Freddy audiobook or ebook root. **[x] implemented** — `organize_apply`'s existing atomic move/trash behavior, unchanged by this feature.
13. Shelfmark triggers an Audiobookshelf scan. **[x] implemented** — `library_scan`, auto-enqueued on a successful organize when Audiobookshelf is configured; otherwise the book waits for the next scheduled scan.
