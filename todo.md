...
**Fixed 2026-09-21. A book went into the library back to front, and someone listened to half of it that way.** A GraphicAudio release of *Fourth Wing* arrived as two files with nothing in common: `EMPYREAN0102.m4b` — the **second** half — beside `Fourth_Wing__The_Empyrean_1__by_Rebecca_Yarros.mp4`, the first. `track_sort_key` orders by filename, "E" sorts before "F", and the wrong half became track `01`. That is not cosmetic: everything downstream reads the first track, so the title came from its album tag (*"The Empyrean 1: Fourth Wing 2 of 2"* — which is what Audiobookshelf then displayed as the book's name), the author came from its artist tag (`Graphic Audio LLC.`, the publisher), the year came from its date tag (2024, part two's), and ABS built a 15-chapter list that played the ending first. The tags knew all along — the two files say *"1 of 2"* and *"2 of 2"*.

New `order_tracks` in `src/main.py` replaces the three bare `sorted(..., key=track_sort_key)` call sites. Filenames still decide whenever they can; the tags are opened only once the filenames have **failed to agree on a pattern** (`name_skeleton` collapses digit runs, so a book ripped in one go reduces to a single skeleton and is never reopened), the book has no disc/index ordering already, and there are between 2 and `PART_PROBE_MAX_TRACKS` (24) files. Every file must then name a part and no two may claim the same one — a partial answer is discarded, because a confidently wrong order is worse than the alphabetical one. `PART_OF_RE` is tried before `PART_LABEL_RE` so *"The Empyrean **Book 1**: Fourth Wing **2 of 2**"* reads as part two, not part one. Covered by `TagPartNumberTests`, `OrderTracksTests` and `FourthWingPlanTests`; seven mutations checked, all caught.

The live book was repaired in place the same day — files swapped, `metadata.json` rewritten (title *Fourth Wing*, author *Rebecca Yarros*, series *The Empyrean #1*, chapters rebuilt in playing order as `Part 1 · 1` … `Part 2 · 8`), folder moved to `Rebecca Yarros/2023 - Fourth Wing`. The ABS item kept its id across the move, so the in-progress listen survived.

- [ ] **Follow-up: the publisher is still taken for the author.** `enrich_meta` fills a missing author from the first track's `artist` tag, which on every GraphicAudio release is `Graphic Audio LLC.`. The real author is sitting in the same file's `copyright` tag (`Copyright © 2023 by Rebecca Yarros`), and often in the release title too. Ordering is fixed, so the *right* file is read now — but it still says the publisher.
